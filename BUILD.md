...
- Tauri 2 tooling via `create-tauri-app` / `tauri` CLI (Tauri 2 is stable and uses a permissions/capabilities system—design around it) :contentReference[oaicite:0]{index=0}
### Recommended
...
Use `create-tauri-app` per Tauri 2 docs. :contentReference[oaicite:1]{index=1}
Example (PowerShell):
```powershell
mkdir OpsPad
cd OpsPad
...
o	If password auth:
	Prefer key-based first; password is tricky to feed securely to ssh.exe
	For MVP, require key auth OR use sshpass-like behavior is NOT acceptable on Windows
